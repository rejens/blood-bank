...
<?php if(isset($_POST['search'])){
   ...
   include "algorithms/searching.php";
...
//   $sql= "select * from donor_details join blood where donor_details.donor_blood=blood.blood_id AND donor_blood='{$bg}' order by rand() limit 5";
   $sql="select * from donor_details,blood where donor_details.donor_blood=blood_id";
   $result=mysqli_query($conn,$sql) or die("query unsuccessful.");
  ...
  if(mysqli_num_rows($result)>0)   {
   //first sorts data using merge sort
    $sorted_data=merge_sort($data);
    //then searches data using binary search
   $searched_data=binarySearch($sorted_data,$bg);
      foreach($searched_data as $row){
      ?>
